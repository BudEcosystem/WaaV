...
- ✅ **STANDS (re-confirmed by primary sources):** frame-rate is fixed per active tick (DSM arXiv 2509.08753: "aligning X and Y to the same frame rate allows batched streaming inference"); AR codec-LM decode is memory-bandwidth-bound with near-linear batch (GB10 §1.1: flat 1→64, 55×@64); the two-batcher **catalog of compute kernels is complete** (no 2025-26 model needs a third *irreducible* kernel); lockstep scales to ~400 streams/H100 (Kyutai); MTP/depth-transformer (not draft-spec-decode) is the right codec multi-token mechanism (PCG arXiv 2511.13732: exact draft spec-decode = **0.98× net slowdown** on acoustic tokens); FlashInfer ~2× regression + ORT-CUDA-can't-int8-GEMM on Blackwell hold. **A fixed-slot non-preemptible engine structurally DODGES vLLM's four worst scars** (preempt-thrash, priority-starvation, 257-token padding, graph-disable cliffs) — the vLLM-core mining confirmed this directly.
...
| Lockstep batching | **55×@64** | the §4 lockstep batcher is *the* throughput mechanism |
