...
3. **"Built" ≠ "shipped" — only LIVE-PATH-INTEGRATED counts (NO shelf-ware).** Every component built — now or earlier — MUST be optimally integrated into the LIVE WaaV Infer main path: reachable from a real server entry point (`ws.rs` / the REST handler / the realtime-S2S handler / `engine.rs`) AND exercised by a **LIVE (non-Fake) integration test** — not test-only, dead, flag-disabled, duplicated, or shadowed by an older path. A component that exists only behind a `FakeStage`/double, or that the live handler never reaches, is a **limitation to WIRE IN bit-faithfully** (then prove with a live test), not a feature. Concretely, the audit list that must each be reachable + live-tested: `codec_ar_batcher` (live codec-AR WS/REST), `stt_coalescer` (live transcribe), `tts_coalescer` (live one-shot TTS), the S2S `DuplexStepModel` (live realtime, a real GPU model — NOT `FakeStage`), the production spine (`FrameWatchdog`/`LeakWatchdog`/`VramAccountant`), the NaN-reject sampler, delta-streaming egress, barge-in, the admission machinery (`DutyLedger`/`Ceilings`/deadline-graded + bounded queue/shed), and the 16-arm registry. When you add a seam, wire it to the live entry point in the SAME change and add the live integration test in the SAME change.
...
- **Load resilience — bound EVERYTHING, shed with a typed signal, never crash/OOM/hang/unbounded-buffer (GATE #9, shipped).** Every admission/egress queue is BOUNDED: the submission channel + per-stream egress are bounded `tokio::mpsc` (never `UnboundedSender` — the literal flood-to-OOM trap), and the in-loop `pending` admission queue is hard-capped (`serve_codec_ar_multiplexed_bounded`). Under a spike, the worker QUEUES (bounded) / SHEDS (typed `AdmissionRejected` 429 + retry-after — surfaced through `InferError`/`TerminalFrame::Error`, never a panic/hang) / BACKPRESSURES (a slow consumer parks the producer briefly, then is dropped with `SlowConsumer` — never an unbounded buffer). Admission is **VRAM-accounted** (`CodecArAdmission` reserves a per-stream KV footprint against the box `VramAccountant` budget — never admit past it → no unified-memory OOM) and **deadline-aware** (the measured `DutyLedger` + `Ceilings` are wired INTO admission, not just scheduling: a saturated bus / a queue too deep to meet the deadline sheds the new request). Admission/shed/backpressure are **control-plane only** — an accepted stream is BIT-IDENTICAL to the single-stream reference (no numerics change). The RED proof is the spike+latency-explosion stress gate (`gate9_request_spike_and_latency_explosion_…`): peak memory bounded, accepted bit-identical, shed typed-signalled.
