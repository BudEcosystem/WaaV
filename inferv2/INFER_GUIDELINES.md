...
| **Batch the AR step** — it's the headline lever (55×@64); size slots at the efficiency knee (~B64 on GB10), not the KV wall | decode is bandwidth-bound (`t_step ≈ WeightBytes/bandwidth`, flat in B); batching fills idle compute for free — the lever the no-quant constraint leaves intact | (the lockstep batcher itself) |
