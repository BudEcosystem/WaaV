...
2. **AR codec‑LM decode is the ideal batching workload and the GPU is idle at batch 1.** Measured on GB10: decode‑step latency is **flat from batch 1→64 (8.6→9.95 ms)** → **55× throughput @64, 84× @128**. One GB10 serves **~64–128 concurrent realtime streams** within a 40–80 ms frame budget. Batching is *nearly free* because decode is memory‑bandwidth/launch‑bound. *(§1.1.)*

3. **Paradigm determines the batch profile — two families, therefore two batchers.** AR/RNNT/MTP decode is **memory‑bound → near‑linear batch** (55×@64). Diffusion/flow/masked‑diffusion/encoder is **compute‑bound → sublinear batch** (measured **~10×@64** for a chunk‑CFM DiT step) with **fixed N‑step solves** that can blow the frame budget. Hybrids nest the two; the **nested per‑frame diffusion patch batches like AR (38×@64)** because a tiny latent is launch‑bound. *(§1.5, paradigm study.)*
...
- **Lockstep batcher** — AR codec‑LM, AR depth/MTP, RNNT/TDT, AR STT decode. Batch on the **stream axis**, fixed slots, per‑stream exec‑mask over a rectangular batch, per‑slot ring KV, advance one frame/tick, wall‑clock paced. Free near‑linear scaling (measured 55×@64). CUDA‑graph the step @ batch‑1/edge; eager/compile @ high‑batch/DC.
