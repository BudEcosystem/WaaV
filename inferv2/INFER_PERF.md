...
**The headline consequence under your no-quant constraint:** AR decode is memory-bandwidth-bound (`t_step ≈ WeightBytes / bandwidth`, flat in batch), so the two levers that shrink `WeightBytes` (quantization) or skip steps (speculation) are exactly the ones you ruled out — **but the lever they leave fully intact, batching, is a 55× win, and it's *more* available precisely because the GPU is idle at batch 1.** Perf comes from **batching + bandwidth physics + selecting the right exact kernel**, not from writing kernels. A custom kernel can't beat the memory roofline that's already binding.
...
| **3** | **Lockstep batching amortization** | **55×@64, 84×@128** | decode flat 8.6→9.95ms B1→64; efficiency 100%(≤B32)→87%@64→66%@128 = the roofline ridge. **The headline lever the no-quant constraint leaves intact.** |
...
2. **Lockstep batching** — amortize the weight stream across B slots (55×@64). The headline throughput lever.
