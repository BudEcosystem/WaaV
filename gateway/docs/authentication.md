...
WaaV Gateway supports two authentication methods for protecting API endpoints:
...
```
┌─────────┐                 ┌───────┐                ┌──────────────┐
│ Client  │                 │ WaaV Gateway │                │ Auth Service │
└────┬────┘                 └───┬───┘                └──────┬───────┘
     │                          │                           │
     │ POST /speak              │                           │
     │ Authorization: Bearer tk │                           │
...
The JWT signed by WaaV Gateway contains the following claims structure:

```json
{
  "sub": "waav-gateway-auth",
  "iat": 1234567890,
  "exp": 1234568190,
  "auth_data": {
...
- `sub` (Subject): Always set to `"waav-gateway-auth"` - identifies the JWT issuer
...
- `x-waav-gateway-*` (internal application headers)
- `host` (infrastructure)
...
WaaV Gateway maps auth service responses to HTTP status codes as follows:

| Auth Service Status | WaaV Gateway Response Status | Description |
|---------------------|----------------------|-------------|
...
1. **Success (200 OK)**
   - Status: `200 OK`
   - Body: Optional (ignored by WaaV Gateway)
...
3. **Server Errors (5xx)**
   - Status: `500`, `503`, etc.
   - Body: Optional error message (capped at 500 chars)
   - Meaning: Temporary auth service issue, WaaV Gateway returns 502
...
def verify_auth_request(jwt_string, public_key):
    try:
        ...
        if payload['sub'] != 'waav-gateway-auth':
...
2. **Network Security**
   ...
   - Use TLS for WaaV Gateway in production
   - Consider mutual TLS between WaaV Gateway and auth service
...
- **Source Authentication**: Auth service knows request comes from legitimate WaaV Gateway instance
...
2. **For JWT mode - Deploy auth service close to WaaV Gateway** to reduce network latency
...
WaaV Gateway's dual authentication approach combines the simplicity of API secrets with the flexibility of JWT validation, giving you the best of both worlds.
