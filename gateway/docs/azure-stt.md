...
WaaV Gateway supports Microsoft Azure Cognitive Services Speech-to-Text for real-time streaming speech recognition using WebSocket bidirectional streaming.
...
Azure STT responses are converted to WaaV Gateway's unified `STTResult` format:
...
The Azure provider maps Speech Service events to WaaV Gateway's unified format:
