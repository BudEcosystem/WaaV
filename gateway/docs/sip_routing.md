...
| **WaaV Gateway** | Receives LiveKit webhooks, routes to customer endpoints based on SIP domain |
...
## WaaV Gateway Webhook Processing

### Domain Extraction Priority

When WaaV Gateway receives a `participant_joined` webhook from LiveKit, it extracts the target domain using this priority order:
...
WaaV Gateway handles various formats that may appear in SIP headers:
...
When WaaV Gateway forwards an event to a customer webhook, it sends a simplified, consistent payload:
...
| Header | Description |
|--------|-------------|
| `X-WaaV Gateway-Signature` | HMAC-SHA256 signature (format: `v1=<hex>`) |
| `X-WaaV Gateway-Timestamp` | Unix timestamp when signature was generated |
| `X-WaaV Gateway-Event-Id` | Original LiveKit event ID for correlation |
| `X-WaaV Gateway-Signature-Version` | Signature scheme version (`v1`) |
...
4. **LiveKit SIP**:
   ...
   - Sends `participant_joined` webhook to WaaV Gateway

5. **WaaV Gateway Processing**:
...
**Symptom**: WaaV Gateway logs show `SIP forwarding failed: no webhook configured for domain`
...
**Symptom**: WaaV Gateway logs show `Skipping SIP forwarding: malformed sip.h.to header`
...
**Symptoms**:
- WaaV Gateway logs show successful forwarding
...
**Debugging Steps**:
1. Check WaaV Gateway logs for HTTP errors
...
1. **High Availability**: Deploy Kamailio and WaaV Gateway in redundant configurations
...
1. **Connection Pooling**: WaaV Gateway maintains HTTP/2 connection pools for efficient webhook delivery
