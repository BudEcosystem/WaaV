...
WaaV Gateway supports Cartesia Text-to-Speech for high-quality speech synthesis using the Sonic voice models via REST API.
...
Cartesia TTS supports pronunciation customization through WaaV Gateway's pronunciation replacement feature:
...
```json
{
  "tts_config": {
    ...
    "pronunciations": [
      ...
      { "word": "WaaV Gateway", "pronunciation": "Say-na" }
    ]
  }
}
```
...
3. **Connection pooling**: WaaV Gateway reuses HTTP connections for multiple requests
