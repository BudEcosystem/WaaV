...
**First message MUST be a configuration message.** This tells WaaV Gateway what voice providers to initialize and how to configure them.
...
```json
{
  ...
  "waav-gateway_participant_identity": "waav-gateway-ai",
  "waav-gateway_participant_name": "WaaV Gateway AI"
}
```

**LiveKit Token Generation:**
If using LiveKit, clients (other than WaaV Gateway) must call `POST /livekit/token` to obtain their own participant tokens:
...
When the WebSocket connection closes (client disconnect, network failure, or server shutdown), WaaV Gateway automatically:
...
These are messages your application sends to WaaV Gateway.
...
These are messages WaaV Gateway sends to your application.
...
| `waav-gateway_participant_identity` | string | WaaV Gateway AI's participant identity in LiveKit room |
| `waav-gateway_participant_name` | string | WaaV Gateway AI's display name in LiveKit room |
...
if (message.type === "error") {
  console.error("WaaV Gateway error:", message.message);
...
**Critical:** The `sample_rate`, `channels`, and `encoding` must **exactly match** the binary audio frames you send. WaaV Gateway does not perform audio format conversion. Mismatches result in garbled transcriptions or errors.
...
Before synthesis, WaaV Gateway replaces occurrences of `word` with `pronunciation` in the text.

**Audio Caching:**

WaaV Gateway automatically caches TTS audio based on a hash of:
- Provider
- Voice ID
- Model
- Audio format
...
| `waav-gateway_participant_identity` | string | No | WaaV Gateway AI's participant identity. Default: `"waav-gateway-ai"` | `"assistant-bot"` |
| `waav-gateway_participant_name` | string | No | WaaV Gateway AI's display name. Default: `"WaaV Gateway AI"` | `"Support Assistant"` |
...
2. Server generates a JWT token for WaaV Gateway AI participant
3. Server connects to the room as `waav-gateway_participant_identity`
...
Control whose audio and messages WaaV Gateway processes:
...
WaaV Gateway's WebSocket API supports multiple integration patterns. Choose based on your application needs.
...
4. Start streaming microphone audio → WaaV Gateway
...
```json
{
  ...
  "livekit": {
    ...
    "enable_recording": true,
    "waav-gateway_participant_identity": "meeting-assistant",
    "waav-gateway_participant_name": "Meeting Assistant",
    "listen_participants": []
  }
}
```
...
6. Participants speak → LiveKit → WaaV Gateway STT → WebSocket STT results
7. Send "speak" command for AI response
8. WaaV Gateway TTS → LiveKit room (all participants hear) + WebSocket (for logging)
...
**Use Case:** Use WaaV Gateway as LiveKit room manager without audio processing. Useful for room automation, recording control, or data channel messaging.
...
Understanding how audio flows through WaaV Gateway helps you optimize for latency and quality.
...
**Without LiveKit:**
```
Your App → WebSocket Binary → WaaV Gateway
                               ↓
...
**With LiveKit:**
```
Browser Participant → LiveKit Room → WaaV Gateway
                                      ↓
...
If compiled with `noise-filter` feature (disabled by default), WaaV Gateway applies DeepFilterNet noise reduction:
...
If compiled with `turn-detect` feature (disabled by default), WaaV Gateway uses an ONNX model to detect when speech ends:
...
**Without LiveKit:**
```
Your App → "speak" command → WaaV Gateway
                              ↓
                         TTS Provider
                              ↓
                         (Check cache)
...
WaaV Gateway caches TTS audio for faster repeated synthesis:
...
WaaV Gateway provides intelligent audio interruption:
...
LiveKit integration transforms WaaV Gateway into a multi-party voice platform.
...
WaaV Gateway uses LiveKit for audio distribution, WebSocket for control:

**LiveKit (WebRTC):**
- Participant audio tracks (STT input)
- WaaV Gateway TTS audio tracks (output to participants)
...
2. WaaV Gateway creates room (or joins existing)
3. WaaV Gateway generates JWT token for itself
4. WaaV Gateway joins room as waav-gateway_participant_identity
5. WaaV Gateway publishes audio track (for TTS output)
6. WaaV Gateway subscribes to participant tracks (for STT input)
7. WaaV Gateway subscribes to data channel
8. WaaV Gateway sends "ready" with room details
```

**Active:**
```
- Participants join with their own tokens (from POST /livekit/token)
- Participants speak → WaaV Gateway STT → WebSocket
- WebSocket sends "speak" → WaaV Gateway TTS → LiveKit (all hear)
- Participants send data → WaaV Gateway → WebSocket "message" event
- WebSocket sends "send_message" → WaaV Gateway → LiveKit data channel
```

**Cleanup:**
```
1. WebSocket disconnects (client or server initiated)
2. WaaV Gateway stops STT/TTS providers
3. WaaV Gateway disconnects from LiveKit room (100ms timeout)
4. WaaV Gateway stops recording (if enabled)
5. WaaV Gateway deletes LiveKit room
```

**Room Deletion:**
WaaV Gateway automatically deletes rooms on disconnect to prevent orphaned rooms consuming resources. If you need persistent rooms, manage room lifecycle separately using LiveKit APIs.
...
Control which participants WaaV Gateway processes:
...
**Effect:**
- WaaV Gateway only transcribes audio from `moderator-1` and `moderator-2`
- Other participants' audio ignored (not sent to STT)
- Only data messages from filtered participants forwarded
- WaaV Gateway still publishes TTS audio to all participants
...
**Important:** 100ms after this message, WaaV Gateway closes the WebSocket. This ensures clean LiveKit session teardown. Re-establish connection if needed.
...
WaaV Gateway publishes to LiveKit data channel → all participants receive (via their LiveKit SDK subscriptions).
...
- Records all room audio/video (not just WaaV Gateway)
...
WaaV Gateway communicates errors without abruptly closing connections, allowing recovery.
...
4. **Participant filtering:** Ensure AI not filtered out.
   ```json
   // If WaaV Gateway identity is "waav-gateway-ai", don't filter it out
   {
     "livekit": {
       "listen_participants": ["user-1", "user-2"]
       // This is for INPUT filtering (who WaaV Gateway listens to)
       // OUTPUT (TTS) always goes to all participants
     }
   }
   ```

5. **Audio track published:** Check WaaV Gateway successfully published track (server logs).
...
The WaaV Gateway WebSocket API provides a powerful foundation for real-time voice applications. Key takeaways:
...
With this foundation, you can build sophisticated voice applications ranging from simple transcription services to complex multi-party voice assistants. Explore the examples, experiment with configurations, and build amazing voice experiences with WaaV Gateway!
...
**Need Help?**
- GitHub Issues: [WaaV Gateway Issues](https://github.com/yourusername/waav-gateway/issues)
- Community Support: [Discussions](https://github.com/yourusername/waav-gateway/discussions)
