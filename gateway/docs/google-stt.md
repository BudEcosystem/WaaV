...
WaaV Gateway supports Google Cloud Speech-to-Text v2 API for real-time streaming speech recognition using gRPC bidirectional streaming.
...
Google STT responses are converted to WaaV Gateway's unified `STTResult` format:
...
The Google provider maps Speech-to-Text v2 events to WaaV Gateway's unified format:
...
Google Speech-to-Text v2 uses "Recognizers" to store reusable configuration. WaaV Gateway supports both implicit and explicit recognizers:

### Implicit Recognizer (Default)

When no `recognizer_id` is specified, WaaV Gateway uses the implicit recognizer (`_`). This is suitable for most use cases and requires no pre-configuration.
