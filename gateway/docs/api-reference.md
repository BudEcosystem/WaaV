# WaaV Gateway Architecture & API Guide

WaaV Gateway is a high-performance, real-time voice server built with Rust, Axum, and Tokio. It unifies multiple speech providers, streams audio/text over WebSockets, exposes a concise REST surface, and can mirror audio into LiveKit rooms for WebRTC participants.
...
| --- | --- | --- |
| `turn-detect` | Enabled | Loads the ONNX turn detector, improving `is_speech_final` timing in STT responses. Required for `waav-gateway init`. |
...
| `waav-gateway_participant_identity` | string | Override for the agent identity (default `waav-gateway-ai`). |
| `waav-gateway_participant_name` | string | Override for the agent display name (default `WaaV Gateway AI`). |
...
| `waav-gateway_participant_identity` | string | Agent identity used inside LiveKit (if applicable). |
| `waav-gateway_participant_name` | string | Agent display name (if applicable). |
...
WaaV Gateway implements a three-tier fallback system to ensure every utterance receives a `speech_final` event:
