...
WaaV Gateway provides comprehensive LiveKit integration with automatic SIP infrastructure provisioning, bidirectional webhook handling, and secure event forwarding. This document covers everything you need to know about integrating WaaV Gateway with LiveKit for real-time voice processing and SIP telephony.
...
3. [Inbound Webhooks (LiveKit → WaaV Gateway)](#inbound-webhooks-livekit--waav-gateway)
4. [SIP Configuration & Auto-Provisioning](#sip-configuration--auto-provisioning)
5. [Outbound Webhooks (WaaV Gateway → Downstream)](#outbound-webhooks-waav-gateway--downstream)
...
3. **Webhook Sent to WaaV Gateway**: LiveKit posts `participant_joined` event with SIP headers
4. **Signature Verification**: WaaV Gateway validates JWT signature from LiveKit
...
## Inbound Webhooks (LiveKit → WaaV Gateway)
...
**SIP Host Header Priority**: When determining the target domain for webhook routing, WaaV Gateway checks attributes in this order:
...
WaaV Gateway logs all webhook events with structured fields:
...
When SIP configuration is enabled and LiveKit API credentials are available, WaaV Gateway **automatically provisions** the required LiveKit infrastructure during startup. This ensures your LiveKit server is ready to receive and route SIP calls without manual setup.

### What Gets Provisioned

**SIP Inbound Trunk**:
- Name: `waav-gateway-{room_prefix}-trunk` (e.g., `waav-gateway-sip--trunk`)
...
**SIP Dispatch Rule**:
- Name: `waav-gateway-{room_prefix}-dispatch` (e.g., `waav-gateway-sip--dispatch`)
- Type: Individual dispatch (creates rooms per caller)
- Room naming: `{room_prefix}{phone_number}` (e.g., `sip-+15551234567`)
- Max participants: Default 3 (caller + WaaV Gateway + optional third party)

### Provisioning Behavior

**Idempotent**: Running WaaV Gateway multiple times won't recreate existing resources. The provisioning code checks for existing trunks/dispatch rules by name and skips creation if they already exist.

**Fail-Fast**: If provisioning fails, WaaV Gateway refuses to start and logs a detailed error message. This prevents accepting SIP calls when infrastructure is incomplete.
...
INFO Successfully provisioned SIP resources
  trunk="waav-gateway-sip--trunk"
  dispatch="waav-gateway-sip--dispatch"
...
```
Content-Type: application/json
X-WaaV Gateway-Signature: v1=<hex_signature>
X-WaaV Gateway-Timestamp: <unix_seconds>
X-WaaV Gateway-Event-Id: <livekit_event_id>
X-WaaV Gateway-Signature-Version: v1
```
...
WaaV Gateway uses the `ReqManager` infrastructure for webhook forwarding:
...
All outbound SIP webhooks from WaaV Gateway are cryptographically signed using HMAC-SHA256. This ensures downstream services can verify authenticity and detect tampering.
...
|--------|--------|-------------|
| `X-WaaV Gateway-Signature` | `v1=<hex>` | HMAC-SHA256 signature (64 hex chars) |
| `X-WaaV Gateway-Timestamp` | `<unix_seconds>` | Request timestamp (seconds since epoch) |
| `X-WaaV Gateway-Event-Id` | `<event_id>` | LiveKit event ID (for correlation/deduplication) |
| `X-WaaV Gateway-Signature-Version` | `v1` | Signature scheme version (future-proofing) |
...
fn verify_waav-gateway_webhook(
    secret: &str,
...
function verifyWaaV GatewayWebhook(req, res, next) {
  const secret = process.env.SIP_HOOK_SECRET;
  const signature = req.headers['x-waav-gateway-signature']; // "v1=abc123..."
  const timestamp = req.headers['x-waav-gateway-timestamp'];
  const eventId = req.headers['x-waav-gateway-event-id'];
...
def webhook():
    # 1. Extract headers
    signature = request.headers.get('X-WaaV Gateway-Signature', '')
    timestamp = request.headers.get('X-WaaV Gateway-Timestamp', '')
    event_id = request.headers.get('X-WaaV Gateway-Event-Id', '')
...
2. **Update WaaV Gateway configuration**:
   ```yaml
   sip:
...
4. **Restart WaaV Gateway** with new secret
...
Configure LiveKit to send webhooks to your WaaV Gateway instance:

```yaml
# livekit.yaml
webhook:
  urls:
    - http://your-waav-gateway-server:3001/livekit/webhook
  api_key: your-api-key
```

Start WaaV Gateway with matching credentials:
```bash
export LIVEKIT_API_KEY=your-api-key
export LIVEKIT_API_SECRET=your-api-secret
cargo run
...
Trigger events by joining a room and watch WaaV Gateway logs.
...
**Prerequisites**:
- [ ] WaaV Gateway server with SIP webhook forwarding configured
- [ ] Downstream webhook consumer endpoint (HTTPS recommended)
- [ ] Access to modify WaaV Gateway configuration
...
**Step 2: Update WaaV Gateway Configuration**
...
2. Update WaaV Gateway configuration (add `hook_secret`)
3. Restart WaaV Gateway
4. Monitor for 24-48 hours

**Step 5: Validation**

```bash
# Check WaaV Gateway logs
grep "Successfully forwarded webhook" /var/log/waav-gateway.log
...
**Symptoms**: Webhook consumer returns 401, WaaV Gateway logs show 401 response

**Causes**:
- Secret mismatch between WaaV Gateway and consumer
...
**Symptoms**: `WARN: HTTP request error` in WaaV Gateway logs
...
### Related WaaV Gateway Documentation
