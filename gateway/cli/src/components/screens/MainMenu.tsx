...
import { CompactSplash, GRADIENTS } from '../branding/index.js';
import { Badge } from '../common/index.js';
import type { ScreenName, GatewayStatus } from '../../types/index.js';
...
export const MainMenu: React.FC<MainMenuProps> = ({ version, onNavigate, onExit }) => {
  const { exit } = useApp();
  const [selectedIndex, setSelectedIndex] = useState(0);
  const [gatewayStatus, setGatewayStatus] = useState<GatewayStatus | null>(null);

  const config = configManager.get();
  const configuredProviderCount = Object.keys(config.providers).length;
    ...
    <Box flexDirection="column" paddingY={1}>
      ...
      <Box marginY={1} paddingX={1}>
        ...
        <Box marginRight={3}>
          ...
          <Text color={configuredProviderCount > 0 ? 'green' : 'yellow'}>
      ...
      <Box flexDirection="row">
        ...
        <Box flexDirection="column" flexGrow={1} borderStyle="single" borderColor="gray" paddingX={2} paddingY={1} marginLeft={1}>
          ...
          {menuItems[selectedIndex]?.id === 'voice' && gatewayStatus && (
            <Box marginTop={1} flexDirection="column">
              <Text dimColor>Current configuration:</Text>
              <Text>• STT: {config.defaults.stt_provider || 'Not set'}</Text>
              <Text>• TTS: {config.defaults.tts_provider || 'Not set'}</Text>
            </Box>
          )}
