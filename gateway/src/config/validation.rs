...
///
/// Ensures that when TLS is enabled:
/// - Certificate file path is provided and file exists
/// - Key file path is provided and file exists
/// - Both files are readable
pub fn validate_tls_config(tls: &Option<TlsConfig>) -> Result<(), Box<dyn std::error::Error>> {
    // TLS is disabled if config is None
    ...
    std::fs::File::open(&config.cert_path).map_err(|e| {
        format!(
    ...
    std::fs::File::open(&config.key_path).map_err(|e| {
        format!(
...
mod tests {
    ...
    fn test_validate_auth_required_blank_jwt_url_is_unset() {
        ...
        let result = validate_auth_required(
            ...
            &auth_signing_key_path,
            &auth_api_secrets,
            // Not a Bud deployment: this case is about a blank JWT URL, and the control plane
            // being absent is what makes the "no authenticator configured" verdict correct.
            false,
        );
