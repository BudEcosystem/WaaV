...
/// Metrics tracking for cache operations using lock-free atomic counters.
///
/// Thread-safety: All operations are thread-safe and lock-free.
/// Memory ordering: Uses Relaxed ordering since counters are independent.
...
impl CacheMetrics {
    ...
    /// Records a cache hit.
    ///
    /// Uses Relaxed ordering since this counter is independent of other operations.
    #[inline]
    ...
    ///
    /// Note: Values are read with Relaxed ordering. In highly concurrent
    /// scenarios, the returned values may be slightly inconsistent with
    /// each other (one counter may reflect a newer state than another).
    /// For monitoring purposes, this is acceptable.
    ...
    /// Calculates hit rate as a percentage (0.0 to 100.0).
    #[inline]
...
impl MemoryCacheBackend {
    ...
    pub fn new(
        ...
        if let Some(max_size) = max_size_bytes {
            // Cap entry size at u32::MAX to prevent overflow (entries > 4GB capped)
            builder = builder.weigher(|_key, value: &Arc<CacheEntry>| {
