//! Inworld Realtime — an OpenAI-PROTOCOL CLONE on the shared S2S scaffold.
//!
//! Inworld's Realtime API speaks the OpenAI GA wire (and bootstraps with
//! `session.created`, like OpenAI), so [`InworldProtocol`] embeds the
//! live-validated [`OpenAiProtocol`](crate::core::realtime::openai) and delegates
//! every wire method to it (byte-identical GA wire), overriding only the
//! endpoint + auth. [`InworldRealtime`] is the thin `RealtimeSession<InworldProtocol>`
//! newtype.
//!
//! # API Reference
//!
//! - Endpoint: `wss://api.inworld.ai/v1/realtime?model=<model>` (BEST-KNOWN — not live-probed)
//! - Auth: `Authorization: Bearer <key>`
//! - Wire: OpenAI GA; bootstraps with `session.created`

mod protocol;

pub use protocol::InworldProtocol;
...
/// Inworld Realtime client — a newtype over the generic [`RealtimeSession`]
/// driver parameterized by [`InworldProtocol`]. Every [`BaseRealtime`] method
/// delegates to the driver.
pub struct InworldRealtime(RealtimeSession<InworldProtocol>);
...
#[async_trait]
impl BaseRealtime for InworldRealtime {
    fn new(config: RealtimeConfig) -> RealtimeResult<Self> {
        Ok(Self(RealtimeSession::<InworldProtocol>::new(config)?))
    }
    ...
    fn get_provider_info(&self) -> serde_json::Value {
        serde_json::json!({
            "provider": "inworld",
            "api_type": "WebSocket Realtime (Inworld)",
            "version": "1.0.0",
            "wire": "OpenAI GA",
            "auth": "Authorization: Bearer",
            "endpoint_status": "best-known (not live-probed)",
            "features": {
...
mod tests {
    ...
    async fn creates_and_reports_provider() {
        let rt = InworldRealtime::new(cfg()).unwrap();
        assert!(!rt.is_ready());
        assert_eq!(rt.get_provider_info()["provider"], "inworld");
    }
