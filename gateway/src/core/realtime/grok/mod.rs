//! Grok / xAI Realtime — an OpenAI-PROTOCOL CLONE on the shared S2S scaffold.
//!
//! xAI's Realtime API is GA-compatible, so [`GrokProtocol`] embeds the
//! live-validated [`OpenAiProtocol`](crate::core::realtime::openai) and delegates
//! every wire method to it (byte-identical GA wire), overriding only the endpoint
//! + the bootstrap event (`conversation.created` ⇒ SessionReady). [`GrokRealtime`]
//! is the thin `RealtimeSession<GrokProtocol>` newtype.
//!
//! # API Reference
//!
//! - Endpoint: `wss://api.x.ai/v1/realtime?model=<model>` (BEST-KNOWN — not live-probed)
//! - Auth: `Authorization: Bearer <key>`
//! - Wire: OpenAI GA-compatible; bootstraps with `conversation.created`

mod protocol;

pub use protocol::GrokProtocol;
...
/// Grok / xAI Realtime client — a newtype over the generic [`RealtimeSession`]
/// driver parameterized by [`GrokProtocol`]. Every [`BaseRealtime`] method
/// delegates to the driver.
pub struct GrokRealtime(RealtimeSession<GrokProtocol>);
...
#[async_trait]
impl BaseRealtime for GrokRealtime {
    fn new(config: RealtimeConfig) -> RealtimeResult<Self> {
        Ok(Self(RealtimeSession::<GrokProtocol>::new(config)?))
    }
    ...
    fn get_provider_info(&self) -> serde_json::Value {
        serde_json::json!({
            "provider": "grok",
            "api_type": "WebSocket Realtime (xAI)",
            "version": "1.0.0",
            "wire": "OpenAI GA-compatible",
...
mod tests {
    ...
    async fn creates_and_reports_provider() {
        let rt = GrokRealtime::new(cfg()).unwrap();
        assert!(!rt.is_ready());
        assert_eq!(rt.get_provider_info()["provider"], "grok");
    }
