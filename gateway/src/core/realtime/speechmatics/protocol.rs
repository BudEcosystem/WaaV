...
impl RealtimeProtocol for SpeechmaticsProtocol {
    ...
    fn connect_spec(&self, cfg: &RealtimeConfig) -> RealtimeResult<ConnectSpec> {
        ...
        let endpoint_url = cfg
            ...
            .map(str::trim)
            .filter(|s| !s.is_empty())
            // Defense-in-depth: only a ws://|wss:// endpoint is honored (matching the
            // `apply_endpoint_override` scheme guard) so a future mis-wire can't
            // downgrade the scheme or point at a non-WS target (review wf_f32563c3 #2).
            .filter(|s| s.starts_with("ws://") || s.starts_with("wss://"))
            .unwrap_or(SPEECHMATICS_FLOW_URL);
        // The SERVER-CONFIG `realtime_endpoint_override` (proxy / self-hosted /
        // gov-cloud / local mock) WINS over BOTH the default AND `cfg.endpoint` when
        // set — the `Bearer` header rides along unchanged. SSRF-safe: never
...
mod tests {
    ...
    /// SERVER-CONFIG `realtime_endpoint_override` WINS over BOTH the default AND
    /// `cfg.endpoint` (the legacy on-prem override), used VERBATIM; the `Bearer`
    /// header rides along. Server-config-only (never client-settable).
    #[test]
    fn connect_spec_server_override_beats_endpoint() {
        let cfg = RealtimeConfig {
            endpoint: Some("wss://flow.example.com/v1/flow".into()),
