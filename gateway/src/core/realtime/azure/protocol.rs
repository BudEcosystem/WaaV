...
impl RealtimeProtocol for AzureProtocol {
    ...
    fn connect_spec(&self, cfg: &RealtimeConfig) -> RealtimeResult<ConnectSpec> {
        ...
        //
        // `build_url()` derives the host from azure's RESOURCE (`cfg.endpoint`).
        // The SEPARATE SERVER-CONFIG `realtime_endpoint_override` (proxy / mock /
        // gov-cloud) WINS over that derived url when set — the `api-key` header
        // rides along unchanged. SSRF-safe: never client-settable.
...
mod tests {
    ...
    /// the resource-derived azure url (`endpoint`), used VERBATIM; the `api-key`
    /// header still rides along. `endpoint` keeps its resource/deployment role.
    /// SECURITY: override is server-config-only (never client-settable).
    #[test]
    fn connect_spec_honors_server_endpoint_override() {
        let cfg = RealtimeConfig {
            endpoint: Some("my-resource".into()), // resource (azure semantics)
        ...
        assert!(headers.iter().any(|(k, v)| k == "api-key" && v == "azkey"));
    }
