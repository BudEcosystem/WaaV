...
impl RealtimeProtocol for HumeProtocol {
    ...
    fn connect_spec(&self, cfg: &RealtimeConfig) -> RealtimeResult<ConnectSpec> {
        ...
        let default_url = self.config.build_websocket_url();
        // SERVER-CONFIG `realtime_endpoint_override` (proxy / self-hosted / gov-cloud
        // / local mock) WINS over the EVI host when set. EVI auths via the QUERY, so
        // the full EVI query (api_key + version + …) is appended to the override when
        // the override carries no query of its own. SSRF-safe: never client-settable.
        let auth_query = default_url.split_once('?').map(|(_, q)| q.to_string());
...
mod tests {
    ...
    /// SERVER-CONFIG `realtime_endpoint_override` WINS over the EVI host; because
    /// EVI auths by QUERY, the FULL EVI query (api_key + voice_id + evi_version) is
    /// APPENDED to the override (which has no query of its own). Server-config-only
    /// (never client-settable).
    #[test]
    fn connect_spec_honors_server_endpoint_override_appends_evi_query() {
        let cfg = RealtimeConfig {
