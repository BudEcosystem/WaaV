...
//!
//! ```rust,no_run
//! use waav_gateway::core::tts::google::TTSGoogleAuthClient;
//! use waav_gateway::core::providers::google::TokenProvider;
//!
//! #[tokio::main]
...
impl TTSGoogleAuthClient {
    ...
    /// ```rust,no_run
    /// use waav_gateway::core::tts::google::TTSGoogleAuthClient;
    /// use waav_gateway::core::providers::google::CredentialSource;
    ///
    /// let source = CredentialSource::ApplicationDefault;
    /// let client = TTSGoogleAuthClient::new(source)?;
    /// # Ok::<(), waav_gateway::core::tts::TTSError>(())
    /// ```
...
///
/// ```rust,ignore
/// use waav_gateway::core::tts::google::{GoogleRequestBuilder, GoogleTTSConfig};
/// use waav_gateway::core::tts::{TTSConfig, provider::TTSRequestBuilder};
///
...
pub struct GoogleTTS {
    ...
    /// HTTP client for API requests (fallback when ReqManager not set)
    http_client: reqwest::Client,
    /// Pooled HTTP client manager (set via set_req_manager)
...
impl GoogleTTS {
    ...
    ///
    /// ```rust,no_run
    /// use waav_gateway::core::tts::{TTSConfig, google::GoogleTTS};
    ///
...
impl BaseTTS for GoogleTTS {
    ...
    async fn speak(&mut self, text: &str, _flush: bool) -> TTSResult<()> {
        ...
        // Use pooled client from ReqManager if available, otherwise use standalone client
    ...
    async fn set_req_manager(&mut self, req_manager: Arc<ReqManager>) {
        *self.req_manager.write().await = Some(req_manager);
        debug!("Google TTS: ReqManager configured for pooled HTTP connections");
    }
}
