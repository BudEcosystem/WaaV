...
/// use waav_gateway::handlers::livekit::extract_sip_attributes;
///
...
///
/// ```
/// use waav_gateway::handlers::livekit::parse_sip_domain;
///
/// // SIP URI formats
...
///
/// # Returns
/// * `Ok(headers)` - Map of signing headers (X-WaaV-Signature, X-WaaV-Timestamp, etc.)
...
fn generate_webhook_signature(
    ...
    headers.insert(
        "X-WaaV-Signature".to_string(),
        format!("v1={}", signature_hex),
    );
    headers.insert("X-WaaV-Timestamp".to_string(), timestamp.to_string());
    headers.insert("X-WaaV-Event-Id".to_string(), event_id.to_string());
    headers.insert("X-WaaV-Signature-Version".to_string(), "v1".to_string());
...
mod tests {
    ...
    fn test_generate_webhook_signature_deterministic() {
        ...
        assert!(headers.contains_key("X-WaaV-Signature"));
        assert!(headers.contains_key("X-WaaV-Timestamp"));
        assert!(headers.contains_key("X-WaaV-Event-Id"));
        assert!(headers.contains_key("X-WaaV-Signature-Version"));

        // Verify header values
        assert_eq!(headers.get("X-WaaV-Timestamp").unwrap(), "1700000000");
        assert_eq!(headers.get("X-WaaV-Event-Id").unwrap(), "EVT_test123");
        assert_eq!(headers.get("X-WaaV-Signature-Version").unwrap(), "v1");

        // Verify signature format (should be v1=<hex>)
        let signature = headers.get("X-WaaV-Signature").unwrap();
        ...
        assert_eq!(
            headers.get("X-WaaV-Signature"),
            headers2.get("X-WaaV-Signature")
        );
    }
    ...
    fn test_generate_webhook_signature_different_timestamps() {
        ...
        assert_eq!(headers1.get("X-WaaV-Timestamp").unwrap(), "1700000000");
        assert_eq!(headers2.get("X-WaaV-Timestamp").unwrap(), "1700000001");
    }
    ...
    fn test_generate_webhook_signature_different_event_ids() {
        ...
        assert_eq!(headers1.get("X-WaaV-Event-Id").unwrap(), "EVT_123");
        assert_eq!(headers2.get("X-WaaV-Event-Id").unwrap(), "EVT_456");
    }
    ...
    fn test_generate_webhook_signature_special_characters() {
        ...
        let headers = result.unwrap();
        assert!(headers.get("X-WaaV-Signature").unwrap().starts_with("v1="));
    }
