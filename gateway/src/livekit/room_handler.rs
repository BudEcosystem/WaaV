...
//!
//! ```rust,no_run
//! use waav_gateway::livekit::room_handler::LiveKitRoomHandler;
//!
//! #[tokio::main]
...
//!     let agent_token = handler.agent_token("my-room", "waav-ai", "WaaV AI")?;
//!
//!     Ok(())
//! }
//! ```
...
impl LiveKitRoomHandler {
    ...
    /// )?;
    ///
    /// let token = handler.agent_token("my-room", "waav-ai", "WaaV AI")?;
    /// # Ok(())
    /// # }
    /// ```
    pub fn agent_token(
    ...
    ///
    /// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
    /// use waav_gateway::livekit::room_handler::RecordingConfig;
    /// let handler = LiveKitRoomHandler::new(
    ///     "http://localhost:7880".to_string(),
    ///     "api_key".to_string(),
    ///     "api_secret".to_string(),
